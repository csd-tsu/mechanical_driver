...
def collision_circles():
    ...
    for j in range(len(scene["entities"])-1):
        ...
        if t_collision:
            ...
            vx1,vy1,vx2,vy2,phi1,phi2 = mech.collision(vx[j][t_collision],\
            vy[j][t_collision], vx[j+1][t_collision], vy[j+1][t_collision],\
            x[j][t_collision], y[j][t_collision], x[j+1][t_collision],\
